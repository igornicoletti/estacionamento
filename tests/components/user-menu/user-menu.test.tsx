...
vi.mock("@/features/my-profile/services/profile-service", async (importOriginal) => {
  const actual = await importOriginal<
    typeof import("@/features/my-profile/services/profile-service")
  >()
  ...
  return {
    ...
    uploadProfileAvatarFile: vi.fn(() =>
      Promise.resolve("test-auth-user/avatar-1.png")
    ),
  }
})
...
describe("UserMenu", () => {
  ...
  it("abre o menu com as ações não redundantes", async () => {
    render(
      <MemoryRouter>
        <UserMenu />
      </MemoryRouter>
    ...
    fireEvent.pointerDown(
      screen.getByRole("button", { name: /abrir menu de usuário/i })
    )
    ...
    expect(
      screen.getByRole("menuitem", { name: "Foto do perfil" })
    ).toBeInTheDocument()
    expect(
      screen.queryByRole("menuitem", { name: "Notificações" })
    ).not.toBeInTheDocument()
    expect(
      screen.queryByRole("menuitem", { name: "Configurações" })
    ).not.toBeInTheDocument()
  })
  ...
  it("atualiza avatar pelo fluxo de upload suportado", async () => {
    ...
    const changePhotoItem = await screen.findByRole("menuitem", {
      name: "Foto do perfil",
    })
    fireEvent.keyDown(changePhotoItem, { key: "Enter" })

    expect(
      await screen.findByRole("heading", { name: "Foto do perfil" })
    ).toBeInTheDocument()
    ...
    await waitFor(() => {
      expect(uploadProfileAvatarFile).toHaveBeenCalledWith(
        file,
        "test-auth-user"
      )
  ...
  it("aceita imagem arrastada para a área da foto", async () => {
    render(
      <MemoryRouter>
        <UserMenu />
      </MemoryRouter>
    ...
    const file = new File(["avatar"], "avatar-drop.png", {
      type: "image/png",
    })
  ...
  it("mostra erro de validação para arquivo de imagem inválido", async () => {
    render(
      <MemoryRouter>
        <UserMenu />
      </MemoryRouter>
