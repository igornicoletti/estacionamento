...
export function DataTable<TData extends RowData, TValue>({
  ...
  const handleSortingChange = React.useCallback<OnChangeFn<SortingState>>(
    (updater) => {
      ...
      } else if (controlledSorting === undefined) {
        setInternalSorting(updater)
      }

      resetPageIndex()
    },
    [controlledSorting, onSortingChange, resetPageIndex]
  )
    ...
    React.useCallback<OnChangeFn<ColumnFiltersState>>(
      (updater) => {
        ...
        if (onColumnFiltersChange) {
          onColumnFiltersChange(sanitizedUpdater)
        } else if (controlledColumnFilters === undefined) {
          setInternalColumnFilters(sanitizedUpdater)
        }
  ...
  const handleGlobalFilterChange = React.useCallback<OnChangeFn<string>>(
    (updater) => {
      ...
      if (onGlobalFilterChange) {
        onGlobalFilterChange(sanitizedUpdater)
      } else if (globalFilterValue === undefined) {
        setInternalGlobalFilter(sanitizedUpdater)
      }

      resetPageIndex()
    },
    [globalFilterValue, onGlobalFilterChange, resetPageIndex]
  )
