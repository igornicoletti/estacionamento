...
function normalizeWhitespace(value: string): string {
  return value.trim().replace(SEARCH_WHITESPACE_PATTERN, " ")
}
...
function dedupeValues<TValue>(values: readonly TValue[]): TValue[] {
...
export function normalizeFilterText(value: unknown): string {
  const searchable = stringifySearchValue(value)
  if (!searchable) return ""

  return normalizeWhitespace(searchable)
    .normalize("NFD")
...
export function isEmptyFilterValue(value: unknown): boolean {
  ...
  if (typeof value === "string") return normalizeSearchValue(value).length === 0
