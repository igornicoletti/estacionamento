const fallbackErrorDetail = "Falha ao processar um item da sincronização."
...
function extractDetailTexts(value: unknown): string[] {
  ...
  return [value.reason, value.message, value.detail, value.error, value.code].flatMap(extractDetailTexts)
}

export function normalizeSyncErrorDetails(value: unknown) {
  return Array.from(new Set(extractDetailTexts(value)))
}
...
export function normalizeSyncHistoryMessage(message: string, status: "success" | "warning" | "failed") {
  ...
  return normalizeReasonText(message) ?? "A sincronização não foi concluída."
}
