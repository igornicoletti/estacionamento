...
import type { SettingsSessionSummary } from "../types/settings-types"
...
function readAuthenticatedAtFromSession(session: unknown) {
  ...
  return typeof lastSignInAt === "string" && lastSignInAt.trim()
    ? lastSignInAt.trim()
    : null
}

export function getLocalSettingsSessionSummary(): SettingsSessionSummary {
...
export async function getCurrentSettingsSession(): Promise<SettingsSessionSummary> {
  const fallback = getLocalSettingsSessionSummary()
