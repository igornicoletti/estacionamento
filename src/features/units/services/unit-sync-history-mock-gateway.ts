...
import { UNIT_SYNC_FAILED_STATUS, UNIT_SYNC_HISTORY_MOCK_STORAGE_SUFFIX, UNIT_SYNC_MOCK_RUN_ID_PREFIX } from "../constants/units-sync"
import { type UnitSyncHistoryEntry } from "../model"
import { limitUnitSyncHistoryEntries, parseStoredUnitSyncHistory } from "./unit-sync-history-normalization"
...
function syncMemoryHistory(entries: readonly UnitSyncHistoryEntry[]) {
  mockSyncHistoryMemoryStore.splice(0, mockSyncHistoryMemoryStore.length, ...limitUnitSyncHistoryEntries(entries))
}
...
function createMockHistoryEntry(input: RecordMockUnitSyncHistoryRunInput): UnitSyncHistoryEntry {
  ...
  return {
    ...
    counters: { received, created: 0, updated: 0, unchanged: hasFailed ? 0 : received, failed: hasFailed ? 1 : 0 },
