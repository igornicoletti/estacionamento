...
const mockUnitsPayload: ErpUnitPayload[] = [
  {
    cod_empresa: 1,
...
function createMockUnitsGateway(): UnitsGateway {
  return {
    async listUnitsPayload() {
      await Promise.resolve()
      return mockUnitsPayload
    },
  }
}

let unitsGateway: UnitsGateway = createMockUnitsGateway()
...
export function resetUnitsGateway() {
  unitsGateway = createMockUnitsGateway()
}
