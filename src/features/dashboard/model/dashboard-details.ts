...
export function getVehicleMovementDetailItems(
  row: DashboardVehicleMovementRow,
  ...
  return [
    ...
    { label: "Câmera", value: row.cameraName },
    {
      ...
      value: formatParkingCameraType(row.cameraType),
    },
    { label: "Data/hora", value: formatDateTime(row.capturedAt) },
    { label: "Confiança", value: `${row.confidence.toFixed(1)}%` },
    {
      label: "Permanência",
      value: row.stayMinutes ? `${row.stayMinutes} min` : "—",
    },
    {
      label: "Status",
      value: formatParkingMovementStatus(row.status),
    },
  ]
}

export function getBillingDetailItems(
  row: DashboardBillingRow,
): readonly AppDetailsSheetItem[] {
  return [
    { label: "Período", value: row.period },
    {
      label: "Veículos cobrados",
      value: row.vehiclesCharged.toLocaleString("pt-BR"),
    },
...
export function getAlertDetailItems(
  row: DashboardAlertRow,
): readonly AppDetailsSheetItem[] {
  return [
    { label: "Severidade", value: formatParkingAlertSeverity(row.severity) },
